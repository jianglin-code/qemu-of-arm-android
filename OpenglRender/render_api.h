 ...
 */
typedef void(*OnPostFn)(void* context ,int width, int height, int ydir,int format, int type ,unsigned char* pixels);
 ...
 *     created.
 *     xywidthheight are the dimensions of the rendering subwindow.
 *     zRot is the rotation to apply on the framebuffer display image.
 */
int createOpenGLSubwindow(FBNativeWindowType window,int x,int y,int width,int height,float zRot);
