...
static int setup_access_params_addr(struct qemu_pipe_dev *dev)
{
	...
	if (!valid_batchbuffer_addr(dev, aps)){
		PIPE_D("valid_batchbuffer_addr failed %d\n",(uint32_t)(paddr));
		return -1;
	}else{
		PIPE_D("valid_batchbuffer_addr ok %d\n",(uint32_t)(paddr));
	}
			     ...
			     struct qemu_pipe *pipe, int *status)
{
	...
	aps->address = address;
	aps->cmd = cmd;
	//PIPE_D("access_with_param %p %p %d\n",(address),(aps->channel),sizeof(unsigned long));
				    ...
				    size_t bufflen, int is_write)
{
    ...
    //PIPE_E("qemu_pipe_read_write begin buffer:%s is_write:%d \n", buffer,is_write);
	while (address < address_end) {
		...
		if (dev->aps == NULL || access_with_param(
			...
			pipe, &status) < 0)
		{
                        PIPE_W("access_with_param  error\n");
			writel((unsigned long)pipe,dev->base + PIPE_REG_CHANNEL);
                       // writel(((unsigned long)pipe)>>32,dev->base + PIPE_REG_CHANNEL_HIGH);
			writel(avail, dev->base + PIPE_REG_SIZE);
			writel(xaddr, dev->base + PIPE_REG_ADDRESS);
                       // writel(((unsigned long)xaddr)>>32,dev->base + PIPE_REG_ADDRESS_HIGH);
	...
	mutex_unlock(&pipe->lock);
	//PIPE_E("qemu_pipe_read_write end buffer:%s is_write:%d \n", buffer,is_write);
out:
	if (ret < 0)
		return ret;
	else
...
long long GetCurrentTimeMS(void)
{
    ...
    long long iDiff=0;
    do_gettimeofday(&now);
    iDiff = (now.tv_sec * 1000LL * 1000LL) + now.tv_usec;
    return iDiff;
}
			      ...
			      size_t bufflen, loff_t *ppos)
{
	//long long t0 = GetCurrentTimeMS();
        ssize_t result =  qemu_pipe_read_write(filp, buffer, bufflen, 0);
        //long long t1 = GetCurrentTimeMS();
        //long long  temp =t1-t0;
        //if(temp > 0)
        //PIPE_E("%lld\t%d\t%lld\t%lld\n",temp,bufflen,t0,t1);
        return result;
}
...
{
        //long long t0 = GetCurrentTimeMS();
        ssize_t result = qemu_pipe_read_write(filp, (char __user *)buffer, bufflen, 1);
	//long long t1 = GetCurrentTimeMS();
        //long long  temp =t1-t0;
        //if(temp > 0)
        //PIPE_E("%lld\t%d\t%lld\t%lld\n",temp,bufflen,t0,t1);
	...
	*/
	...
	for (;;) {
		...
		unsigned long channel = readl(dev->base + PIPE_REG_CHANNEL);
                //PIPE_E("qemu_pipe_interrupt %p\n", channel);
                //unsigned long channel_high = readl(dev->base + PIPE_REG_CHANNEL_HIGH);
                //PIPE_E("qemu_pipe_interrupt %p\n", channel_high);
                //channel |= (channel_high<<32);
                //PIPE_E("qemu_pipe_interrupt %p\n", channel);
		if (channel == 0)
			break;
		...
		pipe  = (struct qemu_pipe *)(ptrdiff_t)channel;
                //PIPE_D("qemu_pipe_interrupt %p\n", pipe);
		/* check if pipe is still valid */
		if ((pipe = radix_tree_lookup(&dev->pipes,
			((unsigned long)pipe&0xFFFFFFFFULL))) == NULL) {
...
static int qemu_pipe_open(struct inode *inode, struct file *file)
{
	...
	*/
	spin_lock_irqsave(&dev->lock, irq_flags);
	if ((ret = radix_tree_insert(&dev->pipes, ((unsigned long)pipe&0xFFFFFFFFULL), pipe))) {
		spin_unlock_irqrestore(&dev->lock, irq_flags);
...
static int qemu_pipe_release(struct inode *inode, struct file *filp)
{
	...
	radix_tree_delete(&pipe_dev->pipes, ((unsigned long)pipe&0xFFFFFFFFULL));
	kfree(pipe);
	spin_unlock_irqrestore(&dev->lock, irq_flags);
...
static int qemu_pipe_probe(struct platform_device *pdev)
{
	...
	PIPE_E("The mapped IO base is %p\n", dev->base);
	...
	setup_access_params_addr(dev);
        ...
        PIPE_E("qemu_pipe_dev_init:dev->version %d \n",dev->version);
	return 0;
...
static struct of_device_id qemu_pipe_match[] = {
        { .compatible = "qemu,pipe", },
        {},
};

MODULE_DEVICE_TABLE(of, qemu_pipe_match);
...
static struct platform_driver qemu_pipe = {
	...
	.driver = {
		.name = "qemu_pipe",
                .owner  = THIS_MODULE,
                .of_match_table = qemu_pipe_match
	}
};
