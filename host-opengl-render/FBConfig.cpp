...
int FBConfig::chooseConfig(FrameBuffer *fb, EGLint * attribs, uint32_t * configs, uint32_t configs_size)
{
    ...
    if (attribs) {
        while (attrib_p[0] != EGL_NONE) {
            ...
            // else if (attrib_p[0] == EGL_ALPHA_SIZE) {
            //     needToAddALPHAAttr = false;
            // }
            attrib_p += 2;
            attribCnt += 2;
        }
    ...
    // if (needToAddALPHAAttr) { + ((needToAddALPHAAttr) ? 2 : 0)
    //     *(attrib_p++) = EGL_ALPHA_SIZE;
    //     *(attrib_p++) = 8;//qom needed!
    // }
