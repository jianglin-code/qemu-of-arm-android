...
/*******************************guoweibin add end************************/
#include "virtio-tp.h"
#define TYPE_VIRTIO_TP_MMIO	"virtio-tp-mmio"
...
typedef struct VirtIOTpMMIO VirtIOTpMMIO;

#define VIRTIO_TP_MMIO(obj) \
        OBJECT_CHECK(VirtIOTpMMIO, (obj), TYPE_VIRTIO_TP_MMIO)
        

static void virtio_tp_mmio_instance_init(Object *obj)
{
    VirtIOTpMMIO *dev = VIRTIO_TP_MMIO(obj);
...
static void virtio_tp_mmio_realize(DeviceState *d, Error **errp)
{
    VirtIOTpMMIO *dev = VIRTIO_TP_MMIO(d);
    DeviceState *vdev = DEVICE(&dev->vdev);
    // qemu_irq irq=dev->parent_obj.irq;
    BusState *bus = d->parent_bus;
...
static void virtio_tp_mmio_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    set_bit(DEVICE_CATEGORY_INPUT, dc->categories);
    dc->realize = virtio_tp_mmio_realize;
    dc->bus_type = TYPE_VIRTIO_MMIO_BUS;
}


static const TypeInfo virtio_tp_mmio_info = {
	.name			= TYPE_VIRTIO_TP_MMIO,
	.parent			= TYPE_VIRTIO_MMIO,
	.instance_size	= sizeof(VirtIOTpMMIO),
	.instance_init	= virtio_tp_mmio_instance_init,
	.class_init		= virtio_tp_mmio_class_init,
};
/*******************************guoweibin 2015/11/27 add end************************/
...
static void virtio_mmio_register_types(void)
{
    ...
    type_register_static(&virtio_msg_mmio_info);
    type_register_static(&virtio_tp_mmio_info);
}
