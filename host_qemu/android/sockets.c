                          ...
                          unsigned     flags )
{
    ...
    list=g_malloc(sizeof(*list)*(count+1));
    //AARRAY_NEW(addr, count);
    addr=g_malloc(sizeof(*addr)*(count));
...
int
socket_connect1( int  fd, const SockAddress*  address )
{
...
int
socket_listen1( int  fd, int  backlog )
{
...
int socket_set_cork1(int fd, int v)
{
...
int  socket_set_nodelay1(int  fd)
{
...
socket_bind_server( int  s, const SockAddress*  to, SocketType  type )
{
    ...
    if (socket_bind(s, to) < 0) {
        printf("could not bind server socket address %s: %s",
          sock_address_to_string(to), errno_str);
        goto FAIL;
    }

    if (type == SOCKET_STREAM) {
        if (socket_listen1(s, 4) < 0) {
            printf("could not listen server socket %s: %s",
              sock_address_to_string(to), errno_str);
            goto FAIL;
        }
    }
...
{
    if (socket_connect1(s, to) < 0) {
        printf( "could not connect client socket to %s: %s\n",
           sock_address_to_string(to), errno_str );
        socket_close(s);
        return -1;
    }
...
socket_accept_any( int  server_fd )
{
    ...
    if (fd < 0) {
        printf( "could not accept client connection from fd %d: %s",
           server_fd, errno_str );
        return -1;
    }
