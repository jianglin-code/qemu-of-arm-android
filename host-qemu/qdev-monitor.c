...
static const QDevAlias qdev_alias_table[] = {
    { "virtio-blk-pci", "virtio-blk", QEMU_ARCH_ALL & ~QEMU_ARCH_S390X & ~QEMU_ARCH_ARM},
    { "virtio-blk-mmio", "virtio-blk", QEMU_ARCH_ARM},
...
/* guoweibin add for static add virtio device */
void virtio_dev_add(const char *type)
{
    ObjectClass *oc = NULL;
    DeviceClass *dc = NULL;
    BusState *bus = NULL;
    ...
    dc = DEVICE_CLASS(oc);    
    if (!dc) {
        printf("[virtio_dev_add] type[%s] error!\n", type);
        return;
    }
    ...
    object_property_set_bool(OBJECT(dev), true, "realized", &err);
    if (err != NULL) {
        printf("[virtio_dev_add] type[%s] realized fail!\n", type);
        object_unparent(OBJECT(dev));
        object_unref(OBJECT(dev));
    }
